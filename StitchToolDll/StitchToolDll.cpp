// StitchToolDll.cpp : 定义 DLL 应用程序的导出函数。
//
#include "stdafx.h"
...
bool CStitchToolDll::read_image(std::vector<std::string> filename_list)
{
	cv::Mat img_temp;
	image.clear();
	...
	{
		img_temp = cv::imread(filename_list[i]);
		if (img_temp.empty()) {
		...
		cv::imshow(std::to_string(i), img_temp);
	}
...
bool CStitchToolDll::read_video(std::vector<std::string> filename_list)
{
	...
	{
		if (!videoCapture.open(filename_list[i])) {
			MessageBox("Read video failed");
			return false;
		}
		video.push_back(videoCapture);
		video[i].set(cv::CAP_PROP_POS_FRAMES, frame_delay[i]);
		//video[i] >> img_temp;
...
bool CStitchToolDll::stitch_image()
{
	if (num_images < 2) {
	...
	std::vector<cv::Mat> resize_img_temp(num_images), full_img_temp(num_images);
	std::vector<cv::Mat> camera_matrix(num_images), distortion_coefficients(num_images);
	...
	for (int i = 0; i < num_images; i++)
	{
		...
		const std::string inputSettingsFile = ("out_camera_data" + std::to_string(i + 1) + ".xml");
		...
		initUndistortRectifyMap(
			camera_matrix[i], distortion_coefficients[i], cv::Mat(),
		...
		remap(image[i], full_img_temp[i], map1[i], map2[i], cv::INTER_LINEAR);
	...
	if (features_type == "surf") finder = cv::makePtr<cv::detail::SurfFeaturesFinder>();
	else if (features_type == "orb") finder = cv::makePtr<cv::detail::OrbFeaturesFinder>();
	else {
		MessageBox("Unknown 2D features type: " +features_type);
		return false;
	}
...
#pragma omp parallel for
	for (int i = 0; i < num_images; ++i) {
		cv::resize(full_img_temp[i], resize_img_temp[i], cv::Size(), feature_scale, feature_scale);
		...
		(*finder)(resize_img_temp[i], features[i]);
	...
	cv::detail::BestOf2NearestRangeMatcher matcher(2, try_cuda, match_conf);
	...
	cv::detail::HomographyBasedEstimator estimator(false);
	cameras.clear();
	...
	if (ba_cost_func == "reproj") adjuster = cv::makePtr<cv::detail::BundleAdjusterReproj>();
		...
		MessageBox("Unknown bundle adjustment cost function:" + ba_cost_func);
		return false;
	}
	...
	if (!warper_creator) {
		MessageBox("Can't create the following warper " + warp_type);
		return false;
	}
	...
	std::vector<cv::Point> corner_temp(num_images);
	for (int i = 0; i < num_images; ++i)
	{
...
#pragma omp parallel for
	for (int i = 0; i < num_images; ++i)
	{
		...
		corner_temp[i] = warper->warp(resize_img_temp[i], K, cameras[i].R, cv::INTER_LINEAR, cv::BORDER_REFLECT, images_warped[i]);
	...
	if (!seam_finder) {
		MessageBox("Can't create the following seam finder '" + seam_find_type);
		return false;
	}
	...
	for (int i = 0; i < num_images; ++i) {
		...
		cv::Size sz = full_img_temp[i].size();
	...
	{
		cv::resize(full_img_temp[img_idx], resize_img_temp[img_idx], cv::Size(), compose_scale, compose_scale);
		cv::Size img_size = resize_img_temp[img_idx].size();

		cv::Mat K;
		cameras[img_idx].K().convertTo(K, CV_32F);

		warper->warp(resize_img_temp[img_idx], K, cameras[img_idx].R, cv::INTER_LINEAR, cv::BORDER_REFLECT, img_warped);

		mask.create(img_size, CV_8U);
		mask.setTo(cv::Scalar::all(255));
		warper->warp(mask, K, cameras[img_idx].R, cv::INTER_NEAREST, cv::BORDER_CONSTANT, mask_warped[img_idx]);
...
bool CStitchToolDll::stitch_video()
{
	cv::VideoWriter outputVideo;
	int flag = 0;
	for (int i = 0; i < video.size(); i++)
		video[i].set(cv::CAP_PROP_POS_FRAMES, frame_delay[i]);
	int ex = static_cast<int>(video[0].get(cv::CAP_PROP_FOURCC));
	if (!outputVideo.open("output.wmv", CV_FOURCC('W', 'M', 'V', '2'), (int)video[0].get(cv::CAP_PROP_FPS),
		cv::Size(img_final_stitched.cols, img_final_stitched.rows), true))
	{
...
#pragma omp parallel for
		for (int img_idx = 0; img_idx < num_images; ++img_idx)
		{
			...
			remap(src, src, map1[img_idx], map2[img_idx], cv::INTER_LINEAR);

			src = cv::Mat(src, cv::Rect(cutX[img_idx], cutY[img_idx], src.cols - 2 * cutX[img_idx], src.rows - 2 * cutY[img_idx]));

			cv::resize(src, src, cv::Size(), compose_scale, compose_scale);

			cv::Mat K;
			cameras[img_idx].K().convertTo(K, CV_32F);

			warper->warp(src, K, cameras[img_idx].R, cv::INTER_LINEAR, cv::BORDER_REFLECT, img_warped);
		...
		cv::Mat result, result_mask, resultVideo;
		blender->blend(result, result_mask);
...
bool CStitchToolDll::sychronize_video(std::vector<std::string> filename_list)
{
	...
	for (int index = 0; index < 4; index++) {
		...
		while (1) {
			...
			if (result[index].size() > frame->sample_rate * sample_time / sample_rate*filter_rate) break;
	...
	for (int i = 0; i < 3; i++) {
		...
		for (int j = 0; j < 400; j++) {//只分析400帧
			...
			for (int k = 0; k < filter_rate*analyse_time; k++) {//比较数据的长度
	...
	int min = *std::min_element(frame_delay.begin(), frame_delay.end());
	for (int i = 1; i < 4; i++) {
		frame_delay[i] += min;
		std::cout << frame_delay[i] << '\t';
	}
	return true;
}


CStitchToolDll::CStitchToolDll()
{
	return;
}